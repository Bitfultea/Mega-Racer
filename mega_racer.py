...
def update(dt, keyStateMap, mouseDelta):
    ...
    '''---------update camera------'''
    g_viewPosition = g_racer.position+[0,0,g_followCamOffset]-g_followCamOffset*g_racer.heading
    g_viewTarget = g_racer.position

    if imgui.tree_node("Camera", imgui.TREE_NODE_DEFAULT_OPEN):
        _,\
        g_followCamOffset = imgui.slider_float("FollowCamOffset ", g_followCamOffset, 2.0, 100.0)
...
# g_renderingSystem.drawObjModel()
...
while not glfw.window_should_close(window):
    ...
    # print(g_racer.update)
    # print(g_racer.update(dt, keyStateMap))
